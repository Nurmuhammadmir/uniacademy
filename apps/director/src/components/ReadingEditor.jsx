...
const ReadingEditor = ({ languageId, levelId, day, initial, onClose, onSaved }) => {
  const { saveReading, uploadContentImage, backendUrl } = useContext(DirectorContext)
  ...
  const seedExercises = () => {
    const src = initial?.exercises || []
    // try to reuse saved exercises by matching the template slot type in order
    const byType = {}
    src.forEach(e => { (byType[e.type] = byType[e.type] || []).push(e) })
    return TEMPLATE.map(type => {
      ...
      if (type === 'sequencing') return { ...base, itemsText: (saved.options || []).join('\n'), orderText: (saved.correct || []).join(', ') }
