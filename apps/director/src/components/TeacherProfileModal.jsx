...
const TeacherProfileModal = ({ teacherId, getTeacherProfile, onClose }) => {
  ...
  return (
    <Modal title='Teacher profile' onClose={onClose} wide>
      {!data ? (
        ...
        <div className='flex flex-col gap-5'>
          <div>
            ...
            <p className='text-muted text-xs mt-1'>{data.teacher.branchId?.name} · working since {new Date(data.employedSince).toLocaleDateString()}</p>
          </div>
          ...
          <div>
            <p className='text-ink font-medium mb-2'>All groups (current + past)</p>
            <div className='flex flex-col gap-1'>
              {data.groups.map(g => (
                <div key={g._id} className='flex justify-between text-sm bg-bg border border-hairline rounded-lg px-3 py-2'>
                  ...
                  <span className='text-muted'>{g.status} · {g.studentIds.length} students</span>
                </div>
              ))}
              {data.groups.length === 0 && <p className='text-muted text-sm'>No groups assigned yet.</p>}
            </div>
          </div>
        </div>
      )}
