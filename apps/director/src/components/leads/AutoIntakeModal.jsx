...
// to the admin/director to manage manually since it doesn't enforce single-ownership itself
const AutoIntakeModal = ({ branchId, subgroup, onClose, onSaved, t }) => {
  const { leadSources, updateLeadSubgroup } = useContext(DirectorContext)
  ...
  const save = async () => {
    const ok = await updateLeadSubgroup(branchId, subgroup._id, { autoIntakeSourceNames: selected })
