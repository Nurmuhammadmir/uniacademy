...
const AddClient = () => {
  const { addClient, clients, updateClient } = useManager()
  const { t } = useLanguage()
  ...
  const existing = id ? clients.find(c => c._id === id) : null
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    if (existing && id) {
      await updateClient(id, { ...form, lat: pin.lat, lng: pin.lng })
    } else {
      await addClient({ ...form, lat: pin.lat, lng: pin.lng })
    }
    setSaving(false)
  ...
  return (
    <div className="p-6 max-w-5xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-display text-gray-900">{existing ? t('addClient.editTitle') : t('addClient.addTitle')}</h1>
      ...
      <form onSubmit={handleSubmit} className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          >
            <Save size={16} />
            {saving ? t('common.saving') : existing ? t('addClient.update') : t('addClient.save')}
          </button>
        </div>
