...
const Students = () => {
  const { allStudents, branches, languages, levels, getLevels } = useContext(SubDirectorContext)
  const { t } = useLanguage()
  ...
  const SELECT_CLS = 'px-3 py-2.5 rounded-xl bg-bg-elevated border border-hairline text-sm transition-colors hover:border-accent/30 focus:outline-none focus:ring-2 focus:ring-accent/30'
  ...
  return (
    <div>
      ...
      <div className='flex flex-wrap gap-3 mb-4'>
        ...
        <select value={branchFilter} onChange={e => setBranchFilter(e.target.value)} className={SELECT_CLS}>
          <option value=''>{t('anyBranch')}</option>
          {branches.map(b => <option key={b._id} value={b._id}>{b.name}</option>)}
        </select>
        <select value={languageFilter} onChange={e => { setLanguageFilter(e.target.value); if (e.target.value) getLevels(e.target.value) }} className={SELECT_CLS}>
          <option value=''>{t('anyLanguage')}</option>
          {languages.map(l => <option key={l._id} value={l._id}>{l.name}</option>)}
        </select>
        <select value={levelFilter} onChange={e => setLevelFilter(e.target.value)} className={SELECT_CLS}>
