...
// zero levels defined at all and still needs a price, and even a course WITH levels charges the
// same price regardless of which level a given group is running
const Pricing = () => {
  const { pricing, upsertPricing, deletePricing, languages } = useContext(SubDirectorContext)
  const { t } = useLanguage()
  const [showCreate, setShowCreate] = useState(false)
  const [editingId, setEditingId] = useState(null)
  const [form, setForm] = useState({ languageId: '', monthlyPrice: '' })

  const openCreate = () => {
    setEditingId(null)
    setForm({ languageId: '', monthlyPrice: '' })
    setShowCreate(true)
  }

  const openEdit = (p) => {
    setEditingId(p._id)
    setForm({ languageId: p.languageId?._id, monthlyPrice: p.monthlyPrice })
    setShowCreate(true)
  }

  const submit = async (e) => {
    e.preventDefault()
    // upsertPricing matches on languageId alone, so editing an existing row and creating a new one
    // both go through the same call - only the pre-filled form values differ
    const ok = await upsertPricing({ ...form, monthlyPrice: Number(form.monthlyPrice) })
    if (ok) { setShowCreate(false); setEditingId(null); setForm({ languageId: '', monthlyPrice: '' }) }
  }
  ...
  return (
    <div>
      ...
      <div className='bg-bg-elevated border border-hairline rounded-2xl overflow-hidden overflow-x-auto'>
        <table className='w-full text-sm'>
          ...
          <tbody>
            ...
            {pricing.length === 0 && (
              <tr><td colSpan={3} className='px-5 py-8 text-center text-muted'>{t('noPricingYet')}</td></tr>
            )}
          </tbody>
        </table>
      </div>
