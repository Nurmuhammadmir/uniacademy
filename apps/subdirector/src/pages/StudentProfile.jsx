      ...
      <div className='flex flex-col gap-5'>
        ...
        <div>
          ...
          <div className='grid grid-cols-1 sm:grid-cols-2 gap-3'>
            {/* only courses the student is CURRENTLY placed in a group for - a left course keeps its
                entry (groupId cleared) purely so the admin app's balance history can still trace what
                was ever billed for it, not something shown here as "currently taking" */}
            {data.courses.filter(c => c.groupId).map(c => (
              <div key={c._id} className='bg-bg-elevated border border-hairline rounded-xl p-4'>
                <div className='flex justify-between items-start mb-1'>
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full ${c.enrollmentStatus === 'active' ? 'bg-accent-soft text-accent dark:bg-[#1E1B4B] dark:text-[#818CF8]' : 'bg-hairline text-muted'}`}>{c.enrollmentStatus === 'active' ? t('active') : t('unpaid')}</span>
                </div>
            ...
            {data.courses.filter(c => c.groupId).length === 0 && <p className='text-muted text-sm col-span-2'>{t('noCoursesYetPlain')}</p>}
          </div>
        </div>
