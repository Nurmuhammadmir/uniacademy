  ...
  return (
    <div>
      <div className='flex flex-wrap gap-1.5 items-center mb-4'>
        ...
        <button onClick={() => setShowManageUnits(true)} className='ml-auto text-xs text-accent font-medium'>Единицы измерения</button>
        <button onClick={() => setShowNewMaterial(true)} className='px-4 py-2 rounded-xl bg-accent text-white text-sm font-medium flex items-center gap-1.5'>
          <Plus size={15} strokeWidth={1.5} /> Материал
        </button>
      </div>
              ...
              editingMaterial?._id === m._id ? (
                <tr key={m._id} className='border-b border-hairline last:border-0'>
                  <td colSpan={5} className='px-4 py-3'>
                    <form onSubmit={submitEditMaterial} className='flex flex-wrap gap-2 items-end'>
                      ...
                      <button type='button' onClick={() => setEditingMaterial(null)} className='px-4 py-2 rounded-lg bg-bg border border-hairline text-muted text-sm font-medium'>Отмена</button>
                    </form>
                  </td>
                </tr>
              ) : (
                <tr key={m._id} className='border-b border-hairline last:border-0'>
                  ...
                  <td className='px-4 py-3 text-right whitespace-nowrap'>
                    <button onClick={() => setEditingMaterial(m)} className='px-3 py-1.5 rounded-lg bg-accent-soft text-accent text-sm font-medium mr-2'>Изменить</button>
                    <button onClick={() => deleteMaterial(m._id)} className='px-3 py-1.5 rounded-lg bg-bg border border-hairline text-muted text-sm font-medium'>Удалить</button>
                  </td>
                </tr>
              )
            ))}
            {filteredMaterials.length === 0 && (
              <tr><td colSpan={5} className='px-4 py-8 text-center text-muted'>Материалов пока нет</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      {showManageCategories && (
        <Modal title='Категории материалов' onClose={() => { setShowManageCategories(false); setEditingCategory(null) }}>
      ...
      {showManageUnits && (
        <Modal title='Единицы измерения' onClose={() => setShowManageUnits(false)}>
          ...
          <form onSubmit={submitNewUnit} className='flex gap-1.5 items-center'>
            <input placeholder='Например: коробка' value={newUnitName} onChange={e => setNewUnitName(e.target.value)}
      ...
      {showNewMaterial && (
        <Modal title='Новый материал' onClose={() => setShowNewMaterial(false)}>
          <form onSubmit={submitNewMaterial} className='flex flex-col gap-3'>
            ...
            <div>
              ...
              <Select value={newMaterial.category} onChange={(v) => setNewMaterial({ ...newMaterial, category: v })} placeholder='Другое'
                options={materialCategories.map(c => ({ value: c.name, label: c.name }))} />
            </div>
            <div>
              <p className='text-xs text-muted mb-1'>Единица измерения</p>
              <Select value={newMaterial.unit} onChange={(v) => setNewMaterial({ ...newMaterial, unit: v })} placeholder='Выберите'
                options={units.map(u => ({ value: u.name, label: u.name }))} />
            </div>
