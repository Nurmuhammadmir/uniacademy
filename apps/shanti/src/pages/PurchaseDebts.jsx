...
const PurchaseDebts = () => {
  ...
  useEffect(() => { getPurchaseDebts(filters).then(d => { if (d) setData(d) }) }, [filters])

  const hasActiveFilters = filters.sellerId || filters.materialId || filters.category
    ...
    <div>
      <div className='flex justify-between items-center mb-4 gap-3 flex-wrap'>
        <div className='bg-amber-50 border border-amber-200 rounded-2xl px-5 py-3 inline-block'>
          ...
          <p className='font-bold tracking-tight text-lg text-amber-700'>{data ? <Money value={data.totalDebt} /> : '—'}</p>
        </div>
          ...
          <tbody>
            {(data?.purchases || []).map(p => (
              <tr key={p._id} className='border-b border-hairline last:border-0 bg-amber-50/40'>
                ...
                <td className='px-4 py-3 font-mono text-amber-700 font-semibold'><Money value={p.amount - p.paidAmount} /></td>
        ...
        {(data?.purchases || []).map(p => (
          <div key={p._id} className='bg-amber-50/60 rounded-xl border border-amber-200 p-4'>
            <div className='flex justify-between items-start'>
              ...
              <p className='text-base font-bold text-amber-700 flex-shrink-0 ml-3'><Money value={p.amount - p.paidAmount} /></p>
            </div>
            <p className='text-xs text-muted mt-2'>{t('amountLabel')}: <Money value={p.amount} /> · {t('paidLabel')}: <Money value={p.paidAmount} /></p>
