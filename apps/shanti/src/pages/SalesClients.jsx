  ...
  return (
    <div>
      <div className='flex flex-wrap gap-1.5 items-center mb-4'>
        <button onClick={() => setCategoryFilter('')} className={`px-2.5 py-1 rounded-full text-xs font-medium border ${!categoryFilter ? 'bg-accent text-white border-transparent' : 'bg-slate-100 border-transparent text-slate-600'}`}>Все</button>
        ...
        <button onClick={() => setShowManageCategories(true)} title='Управление категориями'
        ...
        <button onClick={() => setShowNewClient(true)} className='ml-auto px-4 py-2 rounded-xl bg-accent text-white text-sm font-medium flex items-center gap-1.5'>
          <Plus size={15} strokeWidth={1.5} /> Клиент
        </button>
      </div>
              ...
              editingClient?._id === c._id ? (
                <tr key={c._id} className='border-b border-hairline last:border-0'>
                  <td colSpan={5} className='px-4 py-3'>
                    <form onSubmit={submitEditClient} className='flex flex-wrap gap-2 items-end'>
                      ...
                      <input value={editingClient.phone} onChange={e => setEditingClient({ ...editingClient, phone: e.target.value })} className='px-2 py-1.5 rounded-lg bg-bg border border-hairline text-sm w-40' placeholder='Телефон' />
                      ...
                      <input value={editingClient.comment} onChange={e => setEditingClient({ ...editingClient, comment: e.target.value })} className='px-2 py-1.5 rounded-lg bg-bg border border-hairline text-sm flex-1' placeholder='Комментарий' />
                      <button type='submit' className='px-4 py-2 rounded-lg bg-accent text-white text-sm font-medium'>Сохранить</button>
                      <button type='button' onClick={() => setEditingClient(null)} className='px-4 py-2 rounded-lg bg-bg border border-hairline text-muted text-sm font-medium'>Отмена</button>
                    </form>
                  </td>
                </tr>
              ) : (
                <tr key={c._id} className='border-b border-hairline last:border-0'>
                  ...
                  <td className='px-4 py-3 text-right whitespace-nowrap'>
                    <button onClick={() => setEditingClient(c)} className='px-3 py-1.5 rounded-lg bg-accent-soft text-accent text-sm font-medium mr-2'>Изменить</button>
                    <button onClick={() => deleteClient(c._id)} className='px-3 py-1.5 rounded-lg bg-bg border border-hairline text-muted text-sm font-medium'>Удалить</button>
                  </td>
                </tr>
              )
            ))}
            {filteredClients.length === 0 && (
              <tr><td colSpan={5} className='px-4 py-8 text-center text-muted'>Клиентов пока нет</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      {showManageCategories && (
        <Modal title='Категории клиентов' onClose={() => { setShowManageCategories(false); setEditingCategory(null) }}>
              ...
              <div key={c._id} className='flex items-center justify-between text-sm'>
                {editingCategory?._id === c._id ? (
                  <form onSubmit={submitEditCategory} className='flex gap-1.5 items-center flex-1'>
                    ...
                    <button type='submit' className='px-3 py-1.5 rounded-lg bg-accent text-white text-xs font-medium'>Сохранить</button>
          ...
          {addingCategory ? (
            <form onSubmit={submitNewCategory} className='flex gap-1.5 items-center mt-3'>
              ...
              <input autoFocus placeholder='Название категории' value={newCategory.name} onChange={e => setNewCategory({ ...newCategory, name: e.target.value })}
                className='flex-1 px-2 py-1.5 rounded-lg bg-bg border border-hairline text-xs' />
              <button type='submit' className='px-3 py-1.5 rounded-lg bg-accent text-white text-xs font-medium'>Добавить</button>
            </form>
          ) : (
      ...
      {showNewClient && (
        <Modal title='Новый клиент' onClose={() => setShowNewClient(false)}>
          <form onSubmit={submitNewClient} className='flex flex-col gap-3'>
            ...
            <div>
              ...
              <Select value={newClient.category} onChange={(v) => setNewClient({ ...newClient, category: v })} placeholder='Другое'
